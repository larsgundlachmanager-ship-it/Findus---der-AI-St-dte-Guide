...
if(tailStart<0) throw new Error("no tail");
const tail=s.slice(tailStart);
const seedBlock=s.match(/const seedKulinarik = \\[[\\s\\S]*?\\];/); if(!seedBlock) throw new Error("seeds"); const head=s.slice(0, seedBlock.index+seedBlock[0].length)+"\\n\\n"+"const words = (s) => [...new Set(s.split(/[\\s,]+/).map((x) => x.trim().toLowerCase()).filter((x) => x.length > 1))];\n\nconst MIN_CAT = 342;\n\n";
...
console.log("fills",Object.keys(fills).length,"tail",tailStart);
