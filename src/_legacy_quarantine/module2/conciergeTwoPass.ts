...
import type { FindusLaw } from '../agi/findusLawRegistry';

const VOUCHER_RE =
  /\b(verzehrgutschein|verzehr[\s-]?gutschein|gutschein|essensgutschein|wertgutschein)\b/iu;

/** Speichert Verzehrgutschein als User-Fakt (früher departurePlanOverview). */
function rememberVoucherFromText(text: string): boolean {
  if (!VOUCHER_RE.test(text)) return false;
  const note = text.replace(/\s+/g, ' ').trim().slice(0, 160);
  useOpenQuestionStore.getState().mergeFromPass1({
    facts: [{ key: 'verzehrgutschein', value: note || 'noch einlösen' }],
    subQuestions: [],
    anticipatedFollowUps: ['Verzehrgutschein vor Abflug einlösen?'],
    topicSummary: 'Verzehrgutschein',
    sourceTurn: note.slice(0, 120),
  });
  return true;
}
...
/** Vollständiger Two-Pass-Lauf — Fragen-Modul entfernt, Stub für Legacy-Imports. */
export async function runConciergeTwoPass(_opts: {
  userText: string;
  messages: Array<{ role: string; content: string }>;
  ...
  throw new Error('Modul 2 (Fragen) ist deaktiviert — runConciergeTwoPass nicht verfügbar.');
}
