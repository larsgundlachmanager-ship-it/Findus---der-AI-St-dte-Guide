...
const cacheSrc = readFileSync(
  join(process.cwd(), 'src/services/navigation/tourAheadRouteCache.ts'),
  'utf8',
);
assert(cacheSrc.includes('isTransitRole'), 'Bahnbein nicht per Fuß-OSRM');
assert(cacheSrc.includes('fetchRailLeg') || cacheSrc.includes('fetchOsmRailPath'), 'Gleis-Pfad nachladen');
...
const aheadIdx = html.indexOf("'route-ahead-line'");
const currentIdx = html.indexOf("'route-line'");
const arrowIdx = html.indexOf("'route-arrows'");
const pinIdx = html.indexOf("id: 'route-pins'");
...
assert(html.includes('route-pin-now'), 'Pinnadel-Icon für das aktuelle Ziel');
assert(html.includes("'route-pin-now-' + n"), 'Nummer sitzt im Nadelschild');
assert(html.includes("'icon-anchor': 'bottom'"), 'Nadelspitze sitzt auf der Koordinate');
assert(html.includes("['get', 'icon']"), 'Pin-Bild ohne Text-Glyphs');
assert(html.includes('route-chevron'), 'Abbiegungs-/Richtungspfeile');
assert(html.includes("id: 'route-arrows'"), 'Pfeil-Layer');
assert(html.includes('function toLines'), 'Folgeben als eigene Straßen-Beine');
assert(html.includes('previewPin'), 'nach Überblick auf den nächsten Halt zoomen');
assert(html.includes("3500"), '3–4 s Überblick vor dem Ranzoomen');
assert(html.includes('nav-chip'), 'Stations-Chips auf der Karte');
assert(html.includes('max-content') || html.includes('width:max-content'), 'Chip-Breite folgt dem Text');
assert(html.includes('setRouteChipsHidden'), 'Chips ausblendbar');
