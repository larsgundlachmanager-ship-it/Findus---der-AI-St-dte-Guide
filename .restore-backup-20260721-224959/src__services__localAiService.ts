...
} from './ai/PromptBuilderService';
import type { VisitedHistory } from './ai/types';
import {
...
/**
 * Satz-Stream für einen beliebigen Prompt (Phase 2 Deep Story).
 */
export async function* streamPromptSentences(
  ...
  // Kein SLM — leerer Stream; deepStoryService nutzt Offline-Fallback
}

/**
 * Satz-Stream für Instant-TTS: erster Satz → Audio, Rest parallel.
 * @deprecated Nutze findusNarrationPipeline (3-Phasen).
 */
export async function* generateLocalTourNarrationStream(
  ...
  const prompt = buildPoiContextPrompt(poi, profile, sessionMemory);
  ...
  try {
    ...
    if (llama?.generateStream) {
      let full = '';
      for await (const sentence of llama.generateStream(prompt)) {
        full = full ? `${full} ${sentence}` : sentence;
        yield sentence;
      }
      if (full.trim()) {
        // Persönlichkeit nachziehen nur für Chat-Log — Sätze schon gesendet
        return;
      }
    }
    if (llama) {
  ...
  const offline = buildContextAwareOfflineNarration(poi, profile, sessionMemory);
