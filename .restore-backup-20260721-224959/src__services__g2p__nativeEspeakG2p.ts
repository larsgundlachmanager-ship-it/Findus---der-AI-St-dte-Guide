...
import { normalizeGermanTtsText, ensureSilenceTokens } from './germanTextNormalize';
...
export function mapEspeakIpaToKokoro(ipa: string): string {
  ...
  // Prosodie: en-dash / Ellipsen → Kokoro-Silence
  s = s.replace(/\u2013/g, '\u2014'); // – → —
  s = s.replace(/\.{3,}/g, '\u2026'); // ... → …
...
export async function phonemizeGermanNativeEspeak(
  ...
  if (nativeReady && mod) {
    ...
    const withPauses = ensureSilenceTokens(normalized, mapped);
    if (__DEV__) {
      console.log(
        `[g2p/espeak-native] de IPA (${withPauses.length}): ${withPauses.slice(0, 140)}`,
      );
    }
    return withPauses;
  }
